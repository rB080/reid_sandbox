...
def compute_uni_rbs(i_feats, t_feats, pid, label_hat=None, tau=0.02, margin=0.1, loss_type='TAL', logit_scale=50):
    loss, _ = compute_per_loss(i_feats, t_feats, pid, tau, margin, loss_type, logit_scale)
    loss = (label_hat*loss).sum() 
    if loss_type in ['TAL','TRL']:
        return loss
    else:
        return loss/label_hat.sum() # mean

def compute_rbs(i_feats, t_feats, i_tse_f, t_tse_f, pid, label_hat=None, tau=0.02, margin=0.1, loss_type='TAL', logit_scale=50):

    loss_bgm, sims1 = compute_per_loss(i_feats, t_feats, pid, tau, margin, loss_type, logit_scale)
    loss_tse, sims2 = compute_per_loss(i_tse_f, t_tse_f, pid, tau, margin, loss_type, logit_scale)

    loss_bgm = (label_hat*loss_bgm).sum()
    loss_tse = (label_hat*loss_tse).sum()
    
    if loss_type in ['TAL','TRL']:
        return loss_bgm, loss_tse, sims1
    else:
        return loss_bgm/label_hat.sum(), loss_tse/label_hat.sum(), sims1 # mean
